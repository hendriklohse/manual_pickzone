...
def sample_skus():
    ...
    totes_dist = pd.read_csv("sku_qty_distribution.csv", sep = ";")
    totes_cdf = totes_dist["cumulative_fraction"]
    totes_pdf = totes_dist["fraction"]
    return random.choices(totes_dist["sku_quantity"], weights = totes_pdf)[0]
...
#             k += 1
#         else:
#             # print("else case (should never happen)")
#             return None
#
# def random_sampler(cdf_list, n_samples):
#     t1 = time.time()
#     res = []
...
#     t2 = time.time()
#     print("time elapsed to sample " + str(n_samples) + " times from CDF: " + str(t2 - t1))
#     return res

# print(random_sampler(cdf_list=cdf_list, n_samples=1000))
